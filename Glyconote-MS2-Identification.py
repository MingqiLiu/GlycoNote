...
Para_filename = filedialog.askopenfilename(
    initialdir=(os.getcwd()),
    filetypes=[("MS/MS data", ".mgf")],
    title=("Select MS/MS data"),
)
Parameter_file = ""
...
Parameter_file = filedialog.askopenfilename(
    initialdir=(os.getcwd()),
    ...
    for F__counts in itertools.product(*F__ranges):
        ...
        if F__mass_sum < (mass_max + 5) and N_glycan_check == 0:
            for i in range(charge_max):
                F__combinations.append(
                    combination(
                        F__description, F__composition, F__mass_sum, i + 1, F__count_sum
                    )
                )
    ...
    for line in F__MGF:
        ...
        if "BEGIN" in line:
            ...
            for i in range(F__peaks_no):
                if (
                    F__peak_int[i] > F__peak_threshold1
                    and F__peak_int[i] > F__peak_threshold2
                    and F__peak_mz[i] < (F__precursor + 1)
                ):
                    F__output_MGF.append(
                        str(F__peak_mz[i]) + " " + str(F__peak_int[i]) + "\n"
                    )
...
### check mass accuracy
def Func_mass_accuracy(
    precursor, mass_additional, mass_compare, tolerance, tolerance_unit
):
    F__output = False
...
def Func_add_random(input, range=1.0):
    ...
    ### range - both
    F__output = random.uniform(
        F__input * (1 - F__range / 2), F__input * (1 + F__range / 2)
    )
    ### range - minus
    ...
    while (
        abs(F__output - F__input) < 0.1
        or abs(abs(F__output - F__input) - 1) < 0.1
        or abs(abs(F__output - F__input) - 2) < 0.1
    ):
        F__output = random.uniform(
            F__input * (1 - F__range / 2), F__input * (1 + F__range / 2)
        )
    ...
    for i in range(len(mass_range) - 1):
        ...
        for x in data:
            ...
            if F__mass_min <= F__mass_now <= F__mass_max:
                for j in range(len(distribution_list)):
                    if (
                        distribution_list[j][1]
                        < F__score_now * 100
                        <= distribution_list[j][2]
                    ):
...
File_array_parameter = [
    line.strip() for line in open(Parameter_file).read().splitlines()
]
...
for i, j in enumerate(Para_name_array):
    ...
    if "Parameter" not in j:
        ...
        if int(getvar[2]) > 0:
            if getvar[0] == "H2O":
                MS2_fragment_array.append(
                    ingredient(getvar[0], float(Para_var_array[i]), -1, int(getvar[2]))
                )
            else:
                MS2_fragment_array.append(
                    ingredient(getvar[0], float(Para_var_array[i]), 0, int(getvar[2]))
                )
            MS2_ingredient_array.append(
                ingredient(
                    getvar[0], float(Para_var_array[i]), int(getvar[1]), int(getvar[2])
                )
            )

            if (
                "RED" not in getvar[0]
                and "ADD" not in getvar[0]
                and "H2O" not in getvar[0]
            ):
                Para_composition_glycan = (
                    str(Para_composition_glycan) + str(getvar[0]) + " "
                )
...
MS2_precursor_list = Func_index_database(
    Func_combinations(MS2_ingredient_array, MS2_precursor_mass_max, 1, Para_NGlycan),
    Para_charged_mass,
)
MS2_fragment_list = Func_index_database(
    Func_combinations(MS2_fragment_array, MS2_precursor_mass_max, Test_charge_max),
    Para_charged_mass,
)
...
if Para_filter_diagnostic_ions == "yes":
    MGF_filtered_diagnostic_ions = Func_filter_MGF(
        Para_filename, 0, Para_filter_diagnostic_ions_top
    )
    ...
    while j < len(File_array):
        if "BEGIN" in File_array[j]:
            ...
            MS2_precursor = (
                MS2_pepmass * float(MS2_charge) - (MS2_charge - 1) * Para_charged_mass
            )
            j += 5
    ...
    MGF_filtered_final = Func_filter_MGF(
        Para_filename, Para_filter_sn, Para_filter_top, MGF_list_diagnostic_ions
    )
...
while j < len(File_array):
    ...
    if "BEGIN" in File_array[j]:
        ...
        if MS2_spectra_no_now % 1000 == 0 or MS2_spectra_no_now == MS2_spectra_no:
            print(
                "Processing spectra: # ", MS2_spectra_no_now, " / total", MS2_spectra_no
            )
        ...
        for x in (
            MS2_precursor_list[int(MS2_precursor) - 1]
            + MS2_precursor_list[int(MS2_precursor)]
            + MS2_precursor_list[int(MS2_precursor) + 1]
        ):
            MS2_diag_check = True
            ...
            for i in range(len(MS2_diag_exist)):
                if (
                    MS2_diag_exist[i] == 0
                    and MS2_diag_exist_orig[i] == 0
                    and MS2_diag_desc[i] in x.description
                ):
                    MS2_diag_check = False
                if (
                    MS2_diag_exist[i] == 1
                    and MS2_diag_exist_orig[i] == 1
                    and MS2_diag_desc[i] not in x.description
                ):
                    MS2_diag_check = False
            ...
            if (
                ...
                while "IONS" not in File_array[j] and "=" not in File_array[j]:
                    ...
                    for i in range(1, Test_charge_max + 1):
                        for y in (
                            MS2_fragment_list[int(MS2_peak_now_mz) - 1]
                            + MS2_fragment_list[int(MS2_peak_now_mz)]
                            + MS2_fragment_list[int(MS2_peak_now_mz) + 1]
                        ):
                            if (
                                y.charge == i
                                and y.charge <= MS2_charge
                                and Func_mass_accuracy(
                                    MS2_peak_now_mz,
                                    0,
                                    y.mass,
                                    Para_tolerance_fragment,
                                ...
                                and MS2_peak_now_inten > 0
                            ):
                                MS2_fragment_composition = y.composition
                                if Func_composition_inclusion(
                                    MS2_precursor_composition, MS2_fragment_composition
                                ):
                ...
                if File_result_target_int[MS2_spectra_no_now - 1] == ("0\n"):
                    File_result_target_int[MS2_spectra_no_now - 1] = (
                        str(MS2_precursor)
                        + "\t"
                        + str(
                            "%.3f"
                            % (float(MS2_intensity_sum_real / MS2_intensity_sum_all))
                        )
                        + "\n"
                    )
                elif float(
                    File_result_target_int[MS2_spectra_no_now - 1].split("\t")[1]
                ) < float(MS2_intensity_sum_real / MS2_intensity_sum_all):
                    File_result_target_int[MS2_spectra_no_now - 1] = (
                        str(MS2_precursor)
                        + "\t"
                ...
                if File_result_target_seq[MS2_spectra_no_now - 1] == ("0\n"):
                    File_result_target_seq[MS2_spectra_no_now - 1] = (
                        str(MS2_precursor)
                        + "\t"
                        + str("%.3f" % (float(MS2_seq_cov / (x.count - 1))))
                        + "\n"
                    )
                elif float(
                    File_result_target_seq[MS2_spectra_no_now - 1].split("\t")[1]
                ) < float(MS2_seq_cov / (x.count - 1)):
                    File_result_target_seq[MS2_spectra_no_now - 1] = (
                        str(MS2_precursor)
                        + "\t"
                        + ("%.3f" % (float(MS2_seq_cov / (x.count - 1))))
                        + "\n"
                    )
...
print(
    "Total time for target analysis = ",
    ("--- %.0fs seconds" % (Step2_time_end - Step2_time_start)),
)
print(
    "Maximum time for decoy analysis = ",
    (
        "--- %.0fs seconds"
        % ((Step2_time_end - Step2_time_start) * Decoy_vs_target_repeat)
    ),
    ("(--- %.0fs) seconds" % (Decoy_repeat_max_time)),
)
...
Decoy_round_total = int(
    float(len(FDR_target_data_int)) / float(len(FDR_mass_ranges) - 1) * Para_decoy_round
)
Decoy_repeat = 0
...
while Para_decoy_progress:
        ...
        if "Parameter" not in j:
            ...
            if int(getvar[2]) > 0:
                if getvar[0] == "H2O":
                    MS2_fragment_array.append(
                        ingredient(
                            getvar[0], float(Para_var_array[i]), -1, int(getvar[2])
                        )
                    )
                    MS2_ingredient_array.append(
                        ingredient(
                            getvar[0],
                            float(Para_var_array[i]),
                            int(getvar[1]),
                            int(getvar[2]),
                        )
                    )
                    ...
                    ("Neu" not in getvar[0])
                    and ("ADD" not in getvar[0])
                    and ("RED" not in getvar[0])
                    and ("SiaNAz" not in getvar[0])
                ):
                    MS2_fragment_array.append(
                        ingredient(
                            getvar[0],
                            Func_add_random(Para_var_array[i], Test_random_range),
                            0,
                            int(getvar[2]),
                        )
                    )
                    ...
                    MS2_fragment_array.append(
                        ingredient(
                            getvar[0], float(Para_var_array[i]), 0, int(getvar[2])
                        )
                    )
                    MS2_ingredient_array.append(
                        ingredient(
    ...
    MS2_precursor_list = Func_index_database(
        Func_combinations(
            MS2_ingredient_array, MS2_precursor_mass_max, 1, Para_NGlycan
        ),
        Para_charged_mass,
    )
    MS2_fragment_list = Func_index_database(
    ...
    while j < len(File_array):
        ...
        if "BEGIN" in File_array[j]:
            ...
            if Func_check_decoy(
                MS2_precursor, FDR_mass_ranges, File_result_decoy, Decoy_round_total
            ):
                ### generate combinations for every precursor value
                for x in (
                    MS2_precursor_list[int(MS2_precursor) - 1]
                    ...
                    if (
                        (
                            Func_mass_accuracy(
                                ...
                                0,
                                x.mass,
                                Func_mass_based_range(MS2_precursor),
                                "Da",
                            )
                            or Func_mass_accuracy(
                                MS2_precursor,
                            ...
                            for i in range(1, Test_charge_max + 1):
                                for y in (
                                    ...
                                    if (
                                        ...
                                        MS2_fragment_composition = y.composition
                                        if Func_composition_inclusion(
                                            MS2_precursor_composition,
                                            MS2_fragment_composition,
                                        ):
                                            MS2_intensity_sum_real += MS2_peak_now_inten
                                            MS2_peak_now_inten = 0
                                            MS2_seq_cov_data[y.count - 1] = True
                        ...
                        for i in range(x.count - 1):
                            if MS2_seq_cov_data[i] == True:
                                ...
                                ### record decoy result
                        File_result_decoy = Func_add_decoy(
                            MS2_precursor, FDR_mass_ranges, File_result_decoy
                        )

                        ### output result for current glycan candidate
                        FDR_decoy_data_int.append(
                            str(MS2_precursor)
                            + "\t"
                            + (
                                "%.3f"
                                % (
                                    float(
                                        MS2_intensity_sum_real / MS2_intensity_sum_all
                                    )
                                )
                            )
                            + "\n"
    ...
    for i in range(len(FDR_mass_ranges) - 1):
        Print_decoy_progress[i] = int(
            len(File_result_decoy[i]) * 1000000 / Decoy_round_total
        )
        print(
            "mass range",
            FDR_mass_ranges[i],
    ...
    if Decoy_repeat > Decoy_repeat_max_round:
        Para_decoy_progress = False
    if (
        time.time() - Time_start_decoy
        > ((Step2_time_end - Step2_time_start) * Decoy_vs_target_repeat)
        or time.time() - Time_start_decoy > Decoy_repeat_max_time
    ):
        Para_decoy_progress = False
    ...
    print(
        ...
        Decoy_repeat,
        "  Decoy_time_#",
        ("--- %.0fs" % (time.time() - Time_start_decoy)),
        "\n",
        "Maximum time for decoy analysis = ",
        (
            "--- %.0fs seconds"
...
FDR_decoy_dis_int = Func_score_distribution(
    FDR_decoy_data_int, FDR_mass_ranges, FDR_distribution_list
)
FDR_target_dis_int = Func_score_distribution(
    FDR_target_data_int, FDR_mass_ranges, FDR_distribution_list
)
FDR_decoy_dis_seq = Func_score_distribution(
    FDR_decoy_data_seq, FDR_mass_ranges, FDR_distribution_list
)
FDR_target_dis_seq = Func_score_distribution(
    FDR_target_data_seq, FDR_mass_ranges, FDR_distribution_list
)
...
while i < len(FDR_decoy_array):
    ...
    for j in range(FDR_current_score + 1):
        FDR_decoy_total[FDR_current_score] += float(
            FDR_decoy_array[j + FDR_current_position]
        )
        FDR_target_total[FDR_current_score] += float(
            FDR_target_array[j + FDR_current_position]
        )
    ...
    if FDR_current_score % 100 == 99:
        ...
        while FDR_score_check:
            if FDR_target_total[FDR_score_now] == 0 or FDR_score_now == 0:
                ...
                FDR_decoy_total[FDR_score_now] / FDR_target_total[FDR_score_now]
                < FDR_threshold_int
            ):
                FDR_value.append(FDR_score_now)
                FDR_score_check = False
...
for i in range(len(FDR_value)):
    FDR_final_int.append(
        [FDR_mass_ranges[i], FDR_mass_ranges[i + 1], float(99 - FDR_value[i]) / 100]
    )
...
if Test_output_FDR:
    with open(
        Para_filename.split(".")[0] + "-Result-FDR-Int.txt", "w"
    ) as Output_FDR_report:
        Output_FDR_report.write(
            "Mass_range_1\tMass_range_2\tFDR-Cutoff\t#ID\t#Total\tID/Total\n"
        )
...
if Test_output_distribution:
    with open(
        Para_filename.split(".")[0] + "-Result-dis-int-decoy.txt", "w"
    ) as Output_File_result_decoy_dis:
        for line in FDR_decoy_dis_int:
            Output_File_result_decoy_dis.write(line)

    with open(
        Para_filename.split(".")[0] + "-Result-dis-int-target.txt", "w"
    ) as Output_File_result_target_dis:
...
if Test_output_decoy_list:
    with open(
        Para_filename.split(".")[0] + "-Result-decoy-list.txt", "w"
    ) as Output_decoy_list:
...
if Test_output_FDR:
    with open(
        Para_filename.split(".")[0] + "-Result-FDR-Seq.txt", "w"
    ) as Output_FDR_report:
        Output_FDR_report.write(
            "Mass_range_1\tMass_range_2\tFDR-Cutoff\t#ID\t#Total\tID/Total\n"
        )
...
if Test_output_distribution:
    with open(
        Para_filename.split(".")[0] + "-Result-dis-seq-decoy.txt", "w"
    ) as Output_File_result_decoy_dis:
        for line in FDR_decoy_dis_seq:
            Output_File_result_decoy_dis.write(line)

    with open(
        Para_filename.split(".")[0] + "-Result-dis-seq-target.txt", "w"
    ) as Output_File_result_target_dis:
...
if (
    not os.path.exists(Para_filename.split(".")[0] + "-annotated-spectra")
    and Test_output_spe
):
...
### result headline
File_result.append(
    "spectra\tretention\tprecursor\tglycan mass\tPPM\t"
    + Para_composition_glycan
    + "\tdescription\tcharge\tCov. Int\tCov. Seq\tC13\tTotal MS/MS intensity\n"
)
...
while j < len(File_array):
    ...
    if MS2_precursor_description == "a" and MS2_printed:
        MS2_nonassignable.append(
            str(MS2_title)
            + "\t"
            + str(MS2_pepmass)
            + "\t"
            + str(MS2_rt)
            + "\tDoes not fit any combination\t\t"
            + str(MS2_charge)
            + "\t\n"
        )
        ...
        for x in (
            ...
            if (
                        ...
                        for y in (
                            ...
                            if (
                                ...
                                if Func_composition_inclusion(
                                    ...
                                    MS2_real_mass_exp.append(
                                        File_array[j].split(" ")[0]
                                    )
                                    MS2_real_abundance.append(MS2_peak_now_inten)
                                    MS2_real_mass_theo.append(y.mass)
                                    MS2_real_desc.append(
                                        y.description + ", " + str(y.charge) + " + "
                                    )
                ...
                if Func_FDR_check(
                    ...
                    MS2_precursor, MS2_seq_cov / (x.count - 1), FDR_final_seq
                ):
                    ...
                    if (
                        MS2_title in MS2_identified_title
                        and MS2_cov_int + MS2_cov_seq - MS2_identified_current_score > 0
                    ):
                        MS2_identified_current_score = MS2_cov_int + MS2_cov_seq
...
MS2_fragment_list = Func_index_database(
    Func_combinations(
        MS2_fragment_array, MS2_precursor_mass_max, Test_charge_annotation
    ),
    Para_charged_mass,
)
...
while j < len(File_array):
    ...
    if "BEGIN" in File_array[j]:
        ...
        MS2_spectra_no_now += 1
        if (
            MS2_spectra_no_now % 1000 == 0 or MS2_spectra_no_now == MS2_spectra_no
        ) and not Test_output_spe:
            print(
                "Processing spectra: # ", MS2_spectra_no_now, " / total", MS2_spectra_no
            )

        if (
            MS2_spectra_no_now % 100 == 0 or MS2_spectra_no_now == MS2_spectra_no
        ) and Test_output_spe:
            print(
                "Processing spectra: # ", MS2_spectra_no_now, " / total", MS2_spectra_no
            )
        ...
        for x in (
            ...
            if (
                ...
                if Func_mass_accuracy(
                    ...
                    MS2_c13 = 1
                    File_result_fragment.append(
                        "precursor:\t" + str(MS2_precursor + c13_precursor_shift) + "\n"
                    )
                ...
                File_result_fragment.append(
                    Para_composition_glycan + "\t" + MS2_precursor_composition + "\n"
                )
                File_result_fragment.append(
                    "description:\t" + MS2_precursor_description + "\n"
                )
                File_result_fragment.append("retention:\t" + str(MS2_rt) + "\n")
                File_result_fragment.append("charge:\t" + str(MS2_charge) + "\n")
                File_result_fragment.append(
                    "Mass_matched\tMass_theroetical\tPPM\tAbundance\tComposition: "
                    + Para_composition_glycan
                    + "\tDescription"
                    + "\n"
                )
                ...
                while "IONS" not in File_array[j] and "=" not in File_array[j]:
                    ...
                    if (
                        MS2_peak_now_mz > 300
                        and MS2_peak_now_inten > MS2_intensity_max_300
                    ):
                                ...
                                for i in range(1, Test_charge_annotation + 1):
                                    for v in (
                                        ...
                                        if (
                                            ...
                                            if (
                                                Func_composition_inclusion(
                                                    MS2_precursor_false_composition,
                                                    MS2_fragment_composition,
                                                )
                                                and MS2_peak_now_inten > 0
                                            ):
                                                MS2_false_mass_exp.append(
                                                    float(File_array[j].split(" ")[0])
                                                )
                                                MS2_false_abundance.append(
                                                    MS2_peak_now_inten
                                                )
                                                MS2_false_mass_theo.append(
                                                    float(v.mass)
                                                )
                                                MS2_false_desc.append(
                                                    v.description
                                                    + ", "
                                                    + str(v.charge)
                                                    + " + "
                ...
                for i, r in enumerate(MS2_real_mass_exp):
                    File_result_fragment.append(
                        ("%.4f" % (MS2_real_mass_exp[i]))
                        + "\t"
                        + ("%.4f" % (MS2_real_mass_theo[i]))
                        + "\t"
                        + (
                            "%.1f"
                            % (
                                (MS2_real_mass_exp[i] - MS2_real_mass_theo[i])
                                / MS2_real_mass_theo[i]
                                * 1000000
                            )
                        )
                        + "\t"
                        + ("%.3f" % (float(MS2_real_abundance[i])))
                        + "\t"
                        + MS2_real_comp[i]
                        + "\t"
                        + MS2_real_desc[i]
                ...
                File_result_fragment.append(
                    "Mass_matched_total:\t"
                    + ("%.3f" % (float(MS2_intensity_sum_real)))
                    + "\n"
                )
                ...
                if "yes" in Para_temfalse:
                    ...
                    if not MS2_false_mass_exp:
                        ...
                        for i, r in enumerate(MS2_false_mass_exp):
                            File_result_fragment.append(
                                ("%.4f" % float(MS2_false_mass_exp[i]))
                                + "\t"
                                + ("%.4f" % (float(MS2_false_abundance[i])))
                                + "\t"
                                + (
                                    "%.3f"
                                    % (
                                        float(
                                            float(MS2_false_abundance[i])
                                            / (float(MS2_intensity_sum))
                                        )
                                    )
                                )
                                + "\t"
                                + ("%.3f" % (MS2_false_mass_theo[i]))
                                + "\t"
                                + MS2_false_comp[i]
                                + "\t"
                                + MS2_false_desc[i]
                        ...
                        File_result_fragment.append(
                            "ab.false.total:\t"
                            + ("%.3f" % (float(MS2_intensity_sum_false)))
                            + "\n"
                        )
                ...
                if (str(MS2_title) + str(x.composition)) in MS2_identified_list:
                    File_result.append(
                        ...
                        + "\t"
                        + str(MS2_precursor_composition)
                        + "\t"
                        + str(MS2_precursor_description)
                        + "\t"
                        + str(MS2_charge)
                        + "\t"
                        + str(MS2_cov_int)
                ...
                File_result_fragment.append(
                    "Coverage of intensity\t" + str(MS2_cov_int) + "%\n"
                )
                File_result_fragment.append(
                    "Coverage of sequence\t" + str(MS2_cov_seq) + "%\n"
                )
                File_result_fragment.append("\n\n\n")
                ...
                for i, r in enumerate(Spe_plot_allpeak_mass):
                    if float(Spe_plot_allpeak_abundance[i]) < MS2_intensity_max_300:
                        Spe_plot_allpeak_abundance[i] = float(
                            Spe_plot_allpeak_abundance[i]
                        ) / float(MS2_intensity_max)
                    if float(Spe_plot_allpeak_abundance[i]) >= MS2_intensity_max_300:
                        Spe_plot_allpeak_abundance[i] = float(
                            MS2_intensity_max_300
                        ) / float(MS2_intensity_max)
                ...
                for i, r in enumerate(MS2_real_mass_exp):
                    ...
                    if float(MS2_real_abundance[i]) < MS2_intensity_max_300:
                        Spe_y_list.append(
                            float(MS2_real_abundance[i]) / float(MS2_intensity_max)
                        )
                    if float(MS2_real_abundance[i]) >= MS2_intensity_max_300:
                        Spe_y_list.append(
                            float(MS2_intensity_max_300) / float(MS2_intensity_max)
                        )

                        ### get annotations on real peaks
                    MS2_real_annotation[i] += ", " + str(
                        "%.3f"
                        % (float(MS2_real_abundance[i]) / float(MS2_intensity_max))
                    )
                ...
                if (
                    str(MS2_title) + str(x.composition)
                ) in MS2_identified_list and Test_output_spe:
                    ...
                    Spe_ax1.set_title(
                        ...
                        + str(MS2_cov_seq)
                        + "%"
                        + "      Peak annotation by [description (# of glycans) : charge, m/z, rel. intensity]"
                    )
                    Spe_ax1.set_ylim(
                        [0, 2.2 * MS2_intensity_max_300 / MS2_intensity_max]
                    )
                    ...
                    for i, r in enumerate(Spe_x_list):
                        ...
                        if k == Spe_annotate_line:
                            Spe_y_list_counter = (
                                2.1 * MS2_intensity_max_300 / MS2_intensity_max
                            )
                            k = 0

                        if Spe_annotate_list[i] not in Spe_annotated:
                            Spe_ax1.annotate(
                                Spe_annotate_list[i].replace('_-1', '_loss'),
                                xy=(Spe_x_list[i], Spe_y_list[i]),
                                xytext=(Spe_x_list[i], Spe_y_list_counter),
                                arrowprops=dict(
                                    linestyle="dotted",
                                    facecolor="none",
                                    edgecolor="none",
                                ...
                                rotation="horizontal",
                                fontsize=Spe_annotate_font_size,
                            )
                            Spe_annotated.append(Spe_annotate_list[i])
                            Spe_y_list_counter += (
                                -(1.1 / float(Spe_annotate_line))
                                * MS2_intensity_max_300
                                / MS2_intensity_max
                            )
                            k += 1
...
for i in range(len(Output_library_description_set)):
    ...
    for j in range(Output_library_rt_max + 1):
        ...
        for k in range(len(Output_library_description)):
            if (
                Output_library_description[k] == Output_library_description_set[i]
                and Output_library_rt[k] == j
            ):
...
if Para_filter_diagnostic_ions == "yes" and Test_output_dia_MGF:
    ...
    with open(
        Para_filename.split(".")[0] + "-diagnostic-ions.mgf", "w"
    ) as Output_MGF_diagnostic_ions:
        MGF__output = Func_filter_MGF(
            Para_filename, Para_filter_sn, Para_filter_top, MGF_list_diagnostic_ions
        )
...
messagebox.showinfo(
    title="Greetings",
    message="Spectral analysis finished in "
    + ("--- %.0fs seconds" % (time.time() - Time_start))
    + "\nfor file "
    + Para_filename,
)
